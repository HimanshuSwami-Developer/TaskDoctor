...
async function update(table, id, fields) {
  ...
  await db.query(`UPDATE ${table} SET ${set} WHERE id = $1`, [id, ...keys.map((k) => fields[k])]);
}

// Conditions pointing at a screen that no longer exists lose that target.
function clearDeadBranches() {
  return db.query(`
    UPDATE screens c
    SET branches = (
      SELECT coalesce(jsonb_agg(
        CASE WHEN e.b->>'targetId' IS NOT NULL AND NOT EXISTS (SELECT 1 FROM screens t WHERE t.id = e.b->>'targetId')
             THEN jsonb_set(e.b, '{targetId}', 'null'::jsonb) ELSE e.b END ORDER BY e.ord), '[]'::jsonb)
      FROM jsonb_array_elements(c.branches) WITH ORDINALITY AS e(b, ord)
    )
...
const toComment = (r) => ({
  ...
  assignee: r.assignee, remarks: r.remarks, resolved: r.resolved, createdAt: r.created_at,
});
...
const getScreen = async (id) => toScreen(await one(`${SCREEN_SELECT} WHERE s.id = $1`, [id]));

// ---------------------------------------------------------------- board

app.get('/api/board', route(async (req, res) => {
  const [products, modules, flows, screens, comments] = await Promise.all([
    db.query('SELECT * FROM products ORDER BY sort_order, created_at'),
    db.query('SELECT * FROM modules ORDER BY sort_order, created_at'),
    db.query('SELECT * FROM flows ORDER BY sort_order, created_at'),
    db.query(`${SCREEN_SELECT} ORDER BY s.sort_order, s.created_at`),
  ...
  res.json(products.rows.map((p) => ({ ...toProduct(p), modules: modulesBy[p.id] || [] })));
}));
...
app.patch('/api/products/:id', route(async (req, res) => {
  const name = required(text(req.body.name), 'Name');
  res.json(toProduct(await one('UPDATE products SET name = $2 WHERE id = $1 RETURNING *', [req.params.id, name])));
}));
...
app.post('/api/products/:id/modules', route(async (req, res) => {
  await one('SELECT id FROM products WHERE id = $1', [req.params.id]);
  const name = required(text(req.body.name), 'Name');
  const { rows } = await db.query(
...
app.patch('/api/modules/:id', route(async (req, res) => {
  const name = required(text(req.body.name), 'Name');
  res.json(toModule(await one('UPDATE modules SET name = $2 WHERE id = $1 RETURNING *', [req.params.id, name])));
}));
...
app.post('/api/modules/:id/flows', route(async (req, res) => {
  await one('SELECT id FROM modules WHERE id = $1', [req.params.id]);
  const name = required(text(req.body.name), 'Name');
  const { rows } = await db.query(
...
app.patch('/api/flows/:id', route(async (req, res) => {
  const name = required(text(req.body.name), 'Name');
  res.json(toFlow(await one('UPDATE flows SET name = $2 WHERE id = $1 RETURNING *', [req.params.id, name])));
}));
...
app.put('/api/flows/:id/order', route(async (req, res) => {
  await one('SELECT id FROM flows WHERE id = $1', [req.params.id]);
  const ids = Array.isArray(req.body.ids) ? req.body.ids.filter((x) => typeof x === 'string') : [];
  await db.tx(async (client) => {
    for (const [index, id] of ids.entries()) {
      await client.query('UPDATE screens SET flow_id = $1, sort_order = $2 WHERE id = $3', [req.params.id, index, id]);
    }
  });
  res.json({ ok: true });
}));
...
app.patch('/api/screens/:id', route(async (req, res) => {
  const screen = await one('SELECT * FROM screens WHERE id = $1', [req.params.id]);
  const b = req.body;
  const fields = {};
  ...
  // Pending → no stored date (the sheet shows today). Testing / Complete → date frozen when set.
  if (b.status !== undefined && oneOf(b.status, STATUSES, 'status') !== screen.status) {
  ...
  if (b.branches !== undefined) {
    ...
    const { rows } = await db.query(`SELECT id FROM screens WHERE id = ANY($1) AND type <> 'condition' AND id <> $2`, [wanted, screen.id]);
...
app.post('/api/screens/:id/copy', route(async (req, res) => {
  const source = await one('SELECT * FROM screens WHERE id = $1', [req.params.id]);
  ...
  const { rows: found } = await db.query('SELECT id FROM flows WHERE id = ANY($1)', [flowIds]);
      ...
      if (req.body.withIssues) {
        await client.query(
          `INSERT INTO comments (id, screen_id, text, priority, assignee, remarks)
           SELECT gen_random_uuid()::text, $1, text, priority, assignee, remarks
           FROM comments WHERE screen_id = $2 AND NOT resolved`,
          [id, source.id]);
      }
      created.push(id);
    }
...
app.post('/api/screens/:id/comments', route(async (req, res) => {
  await one('SELECT id FROM screens WHERE id = $1', [req.params.id]);
  const { rows } = await db.query(
    `INSERT INTO comments (id, screen_id, text, priority, assignee, remarks) VALUES ($1, $2, $3, $4, $5, $6) RETURNING *`,
    [
      randomUUID(),
      req.params.id,
      required(text(req.body.text, 1000), 'Issue'),
      PRIORITIES.includes(req.body.priority) ? req.body.priority : 'medium',
      text(req.body.assignee, 60),
...
app.patch('/api/comments/:id', route(async (req, res) => {
  ...
  if (b.assignee !== undefined) fields.assignee = text(b.assignee, 60);
  if (b.remarks !== undefined) fields.remarks = text(b.remarks, 1000);
  await one('SELECT id FROM comments WHERE id = $1', [req.params.id]);
